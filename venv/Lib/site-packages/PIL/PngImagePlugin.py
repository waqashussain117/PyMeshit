...
class ChunkStream:
    ...
    def call(self, cid: bytes, pos: int, length: int) -> bytes:
        ...
        logger.info("STREAM %r %s %s", cid, pos, length)
...
class PngStream(ChunkStream):
    ...
    def chunk_iCCP(self, pos: int, length: int) -> bytes:
        ...
        i = s.find(b"\0")
        logger.info("iCCP profile name %r", s[:i])
        comp_method = s[i + 1]
        logger.info("Compression method %s", comp_method)
        if comp_method != 0:
    ...
    def _open(self) -> None:
        ...
        while True:
            ...
            try:
                ...
                logger.info("%r %s %s (unknown)", cid, pos, length)
                s = ImageFile._safe_read(self.fp, length)
                if cid[1:2].islower():
