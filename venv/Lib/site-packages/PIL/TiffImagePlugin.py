...
class ImageFileDirectory_v2(_IFDv2Base):
        ...
        try:
            ...
            for i in range(tag_count):
                ...
                try:
                    ...
                    logger.info("%s - unsupported type %s", msg, typ)
    ...
    def tobytes(self, offset: int = 0) -> bytes:
        ...
        for tag, value in sorted(self._tags_v2.items()):
            ...
            typ = self.tagtype[tag]
            logger.info("Tag %s, Type: %s, Value: %s", tag, typ, repr(value))
        ...
        for tag, typ, count, value, data in entries:
            logger.info("%s %s %s %s %s", tag, typ, count, repr(value), repr(data))
            result += self._pack(
...
class TiffImageFile(ImageFile.ImageFile):
    ...
    def _open(self) -> None:
        ...
        logger.info("*** TiffImageFile._open ***")
        logger.info("- __first: %s", self.__first)
        logger.info("- ifh: %s", repr(ifh))  # Use repr to avoid str(bytes)
    ...
    def _seek(self, frame: int) -> None:
        ...
        while len(self._frame_pos) <= frame:
            ...
            logger.info("Loading tags, location: %s", self.fp.tell())
    ...
    def _load_libtiff(self) -> Image.core.PixelAccess | None:
        ...
        if hasattr(self.fp, "getvalue"):
            ...
            logger.info("have getvalue. just sending in a string from getvalue")
            n, err = decoder.decode(self.fp.getvalue())
        elif fp:
            # we've got a actual file on disk, pass in the fp.
            logger.info("have fileno, calling fileno version of the decoder.")
            ...
            logger.info("don't have fileno or getvalue. just reading")
            self.fp.seek(0)
    ...
    def _setup(self) -> None:
        ...
        logger.info("*** Summary ***")
        logger.info("- compression: %s", self._compression)
        logger.info("- photometric_interpretation: %s", photo)
        logger.info("- planar_configuration: %s", self._planar_configuration)
        logger.info("- fill_order: %s", fillorder)
        logger.info("- YCbCr subsampling: %s", self.tag_v2.get(YCBCRSUBSAMPLING))
        ...
        logger.info("- raw mode: %s", rawmode)
        logger.info("- pil mode: %s", self.mode)
        ...
        if self.use_load_libtiff:
            ...
            logger.info("- unsupported data organization")
            msg = "unknown data organization"
            raise SyntaxError(msg)
...
def _save(im: Image.Image, fp: IO[bytes], filename: str | bytes) -> None:
    ...
    if "tiffinfo" in encoderinfo:
        ...
        info = {}
    logger.info("Tiffinfo Keys: %s", list(info))
    ...
    if libtiff:
        ...
        logger.info("Saving using libtiff encoder")
        logger.info("Items: %s", sorted(ifd.items()))
        _fp = 0
        ...
        logger.info("Converted items: %s", sorted(atts.items()))
