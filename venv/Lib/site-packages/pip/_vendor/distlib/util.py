    ...
    def ensure_removed(self, path):
        if os.path.exists(path):
            if os.path.isdir(path) and not os.path.islink(path):
                logger.info('Removing directory tree at %s', path)
                if not self.dry_run:
                    shutil.rmtree(path)
                if self.record:
                ...
                if os.path.islink(path):
                    ...
                    s = 'file'
                logger.info('Removing %s %s', s, path)
                if not self.dry_run:
                    os.remove(path)
                if self.record:
...
def _get_external_data(url):
    ...
    try:
        ...
        if not ct.startswith('application/json'):
            logger.info('Unexpected response for JSON request: %s', ct)
        else:
    ...
    def publish(self, event, *args, **kwargs):
        ...
        for subscriber in self.get_subscribers(event):
            try:
                ...
                value = None
            result.append(value)
        logger.info('publish %s: args = %s, kwargs = %s, result = %s', event,
                     args, kwargs, result)
        return result
        ...
        def connect(self):
            ...
            if self.ca_certs and self.check_domain:
                try:
                    ...
                    logger.info('Host verified: %s', self.host)
