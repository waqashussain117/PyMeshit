...
class Wheel(object):
    ...
    def build_zip(self, pathname, archive_paths):
        with ZipFile(pathname, 'w', zipfile.ZIP_DEFLATED) as zf:
            for ap, p in archive_paths:
                logger.info('Wrote %s to %s in wheel', p, ap)
                zf.write(p, ap)
            ...
            try:
                for zinfo in zf.infolist():
                    ...
                    if lib_only and u_arcname.startswith((info_pfx, data_pfx)):
                        logger.info('lib_only: skipping %s', u_arcname)
                        continue
                ...
                if lib_only:
                    logger.info('lib_only: returning None')
                    dist = None
                else:
                    # Generate scripts
    ...
    def mount(self, append=False):
        ...
        if pathname in sys.path:
            logger.info('%s already in path', pathname)
        else:
            if append:
    ...
    def unmount(self):
        ...
        if pathname not in sys.path:
            logger.info('%s not in path', pathname)
        else:
    ...
    def update(self, modifier, dest_dir=None, **kwargs):
        ...
        def update_version(version, path):
            ...
            if updated:
                ...
                logger.info('Version updated from %r to %r', version, updated)
