...
def triangulate_with_triangle(points_or_surface, gradient=2.0):
    ...
    # Update gradient control with feature points and sizes
    ...
    if gradient < 1.0:
        # For small gradients, use smaller triangles
        area_constraint = area_constraint * (1.0 - 0.5 * (1.0 - gradient))
    elif gradient > 1.0:
        # For large gradients, allow larger triangles
        area_constraint = area_constraint * (1.0 + 0.5 * (gradient - 1.0))
    
    # Set up custom refinement function similar to triunsuitable in MeshIt
    def refinement_func(vertices, area):
        '''Custom refinement function that mimics MeshIt's triunsuitable function'''
        ...
        mean_edge = (edge1 + edge2 + edge3) / 3.0
        
        # Check if mean edge length exceeds the mesh size
        if mean_edge**2 > base_size**2:
            return True  # Triangle is unsuitable
        
        # Check against feature points
        grad_sq = gradient**2
        base_size_sq = base_size**2
        
        for i in range(len(feature_points)):
            refine_size_sq = feature_sizes[i]**2
            point = feature_points[i]
            
            # Calculate squared distance from centroid to feature point
            sq_dist = np.sum((centroid - point)**2)
            
            # Apply the MeshIt gradient refinement criterion
            if sq_dist < grad_sq * (base_size_sq - refine_size_sq):
                if mean_edge**2 > (sq_dist / grad_sq + refine_size_sq):
                    return True  # Triangle is unsuitable
        
        return False  # Triangle is suitable
    
    # Run Triangle with quality constraints
    import triangle
    
    # Set up triangle options based on gradient settings
    triangle_opts = f'q{min_angle}a{area_constraint}'
    
    # Include custom refinement if gradient is non-uniform
    if gradient != 1.0 and len(feature_points) > 0:
        # Note: Triangle library in Python doesn't directly support custom refinement
        # functions like the C version. We would need to modify the Triangle library
        # or use a different approach for custom refinement.
        print(f"Using gradient {gradient} with {len(feature_points)} feature points")
        print("Custom refinement would be applied in the C++ version")
